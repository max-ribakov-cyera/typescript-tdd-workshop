...
import { InMemoryPostgresUserServiceRepository } from '../in-memory-postgres';

describe(UserService, () => {
  const logger = mock<Logger>();
  const timeServer = mock<TimeServer>();
  const userService = new UserService(
    new InMemoryPostgresUserServiceRepository(),
    timeServer,
    logger
  );

  beforeEach(() => {
    timeServer.getCurrentTime.mockClear();
  });
  ...
  it('should store and retrieve created users by email', async () => {
    ...
    const maybeUser = await userService.findByEmail(email);
  ...
  it('learning test just to see how tslog looks', async () => {
    const userServiceWithTslogLogger = new UserService(
      new InMemoryUserServiceRepository(),
      timeServer,
      new TslogLooger()
    );
