import { toUser, UserService } from '../user-service';
...
import { SQLiteUserServiceRepository } from '../in-memory-db';

describe(UserService, () => {
  const logger = mock<Logger>();
  const timeServer = mock<TimeServer>();
  const sqLiteUserServiceRepository = new SQLiteUserServiceRepository();
  const userService = new UserService(sqLiteUserServiceRepository, timeServer, logger);
  ...
  describe('product', () => {
    ...
    it('should store and retrieve created users by email', async () => {
      ...
      const maybeUser = await userService.findByEmail('retrive@byEmail.com');

      expect(maybeUser).toMatchObject(created);
    });
    ...
    it('should create user with status Pending and allow to update it', async () => {
      ...
      await userService.activateUser(user.email);

      const maybeUser = await userService.findByEmail('pending@user.com');
    ...
    it.skip('should add email service', () => {
    ...
    beforeAll(async () => {
      const timeToPopulate = await trackDuration(async () => {
        ...
        for (let i = 0; i < numOfUsers; i += chunkSize) {
          const chunk = Array.from({ length: chunkSize }, async (_, j) => {
            return await toUser(
              aUserCreationParams({
          ...
          userPromises.push(sqLiteUserServiceRepository.addUsers(await Promise.all(chunk)));
        }
